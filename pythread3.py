...
def main(args):
    ...
    for i in range(0, args.numberofthreads):
        ...
        t = threading.Thread(name='Thread%d' % i, target=fibonacci, args=(i, random.randint(1500,2000)))
        t.start()
        threads.append(t)
    for t in threads:
        t.join()
...
if __name__ == "__main__":
    logging.basicConfig(format='%(asctime)s = %(message)s', filename='pythread3.log', level=logging.DEBUG, datefmt='%Y-%m-%d %H:%M:%S')
    main(get_args())
    logging.shutdown()
